...
본 연구에서는 PhysioNet에서 제공하는 두 공개 ECG 데이터셋을 사용합니다.
...
> ⚠️ 라이선스 및 용량 문제로 **원본 데이터는 GitHub에 포함하지 않습니다.**  
> 사용자는 PhysioNet에서 직접 데이터를 다운로드한 뒤, `configs/` 또는 환경변수로 경로를 지정해야 합니다.

---

## 🧪 전처리 파이프라인

두 데이터셋(MIMIC-IV-ECG, PTB-XL)에 **동일한 전처리**를 적용합니다.
...
4. **0.5–60 Hz 대역통과 필터(Bandpass Filter)**
...
모델 입력은 다음과 같은 **6가지 리드 조합(Combo)** 중 하나입니다.
...
> 최소 리드 구성에서의 수렴 여부 및 과적합을 모니터링하였습니다.

---

## 🧠 모델 구조

본 프로젝트의 기본 모델은 **ResNet–U-Net 구조의 1차원 인코더–디코더 네트워크**입니다.
...
### 🔢 C1–C6 조합별 전체 12리드 성능 (PTB-XL 기준)

12리드별 지표를 평균한 **조합별 결과**는 다음 범위에 분포합니다.

- 평균 **MSE**: `0.0120 – 0.0174`
- 평균 **RMSE**: `0.0705 – 0.0949`
- 평균 **PCC**: `0.8705 – 0.9768`
- 평균 **SSIM**: `0.9548 – 0.9695`
